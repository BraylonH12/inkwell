...
- **authorId is stored directly on Post.** Each post has exactly 
one author, so no join table is needed; a join table would
only be justified if posts could have multiple authors.
- **AuthService, not User, owns password hashing and verification.**
 Keeps User focused on representing user data; hashing method
  can change later without touching User.